...
def get_user_goals(user_id, db: Session):
    from database.models.user_goal_setup import UserGoal
    goal = db.query(UserGoal).filter_by(user_id=user_id).first()
    if goal:
        return {
...
def process_llm_log(db: Session, user_id: UUID, llm_data: dict) -> DailyNutrition:
    ...
    items = llm_data.get("parsed_data", [])

    # Fallback: if LLM returned no items but has nutrition totals, create one entry
    if not items and (calories > 0 or protein > 0):
        items = [{
            "name": llm_data.get("input", "Unknown"),
    ...
    if intent == "food":
        ...
        # Exercise tracks burned calories independently — does NOT affect remaining food calories
